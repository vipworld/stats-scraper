...
const LEAGUE_URL = 'https://www.basketball-reference.com/leagues/NBA_stats_per_game.html';
...
const toFloat = (stat) => {
  const tmpStat = parseFloat(stat);
  if (!_.isNaN(tmpStat)) return tmpStat;
  else return stat;
};

const convertStatTypes = ({ ...stats }) => {
  for (let stat in stats) {
    stats[stat] = toFloat(stats[stat]);
  }
  return stats;
};
  ...
  const processPages = (genUrl) => {
    ...
    getPageContent(url).then((page) => {
      ...
      if (page.hasNext) {
        ...
        const ret = stats.map((player) => {
          return convertStatTypes(player);
        });
        cb(ret);
      }
    });
...
exports.runPace = (cb) => {
  return getPageContent(PACE_URL).then((page) => {
    cb(page.content);
  });
};

exports.runLeague = (cb) => {
  return getPageContent(LEAGUE_URL).then((page) => {
    const leagues = page.content;
    leagues.map((league) => {
      return convertStatTypes(league);
    });
    cb(leagues);
  });
};
